...
function typosquattingNpmChallenge() {
    models.Feedback.findAndCountAll({ where: { comment: { [Op.like]: '%epilogue-js%' } } }).then(({ count }) => {
        if (count > 0) {
            utils.solve(challenges.typosquattingNpmChallenge);
        }
    });
    models.Complaint.findAndCountAll({ where: { message: { [Op.like]: '%epilogue-js%' } } }).then(({ count }) => {
        if (count > 0) {
            utils.solve(challenges.typosquattingNpmChallenge);
        }
    });
...
function typosquattingAngularChallenge() {
    models.Feedback.findAndCountAll({ where: { comment: { [Op.like]: '%anuglar2-qrcode%' } } }).then(({ count }) => {
        if (count > 0) {
            utils.solve(challenges.typosquattingAngularChallenge);
        }
    });
    models.Complaint.findAndCountAll({ where: { message: { [Op.like]: '%anuglar2-qrcode%' } } }).then(({ count }) => {
        if (count > 0) {
            utils.solve(challenges.typosquattingAngularChallenge);
        }
    });
...
function hiddenImageChallenge() {
    models.Feedback.findAndCountAll({ where: { comment: { [Op.like]: '%pickle rick%' } } }).then(({ count }) => {
        if (count > 0) {
            utils.solve(challenges.hiddenImageChallenge);
        }
    });
    models.Complaint.findAndCountAll({ where: { message: { [Op.like]: '%pickle rick%' } } }).then(({ count }) => {
        if (count > 0) {
            utils.solve(challenges.hiddenImageChallenge);
        }
    });
...
function supplyChainAttackChallenge() {
    models.Feedback.findAndCountAll({ where: { comment: { [Op.or]: eslintScopeVulnIds() } } }).then(({ count }) => {
        if (count > 0) {
            utils.solve(challenges.supplyChainAttackChallenge);
        }
    });
    models.Complaint.findAndCountAll({ where: { message: { [Op.or]: eslintScopeVulnIds() } } }).then(({ count }) => {
        if (count > 0) {
            utils.solve(challenges.supplyChainAttackChallenge);
        }
    });
