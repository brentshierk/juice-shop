...
async function createUsers() {
    ...
    await Promise.all(users.map(async ({ username, email, password, customDomain, key, role, deletedFlag, profileImage, securityQuestion, feedback, address, card, totpSecret = '' }) => {
        try {
...
async function createWallet() {
    const users = await loadStaticData('users');
    return await Promise.all(users.map((user, index) => {
...
async function createRandomFakeUsers() {
    ...
    return await Promise.all(new Array(config.get('application.numberOfRandomFakeUsers')).fill(0).map(() => models.User.create({
        email: getGeneratedRandomFakeUserEmail(),
        password: makeRandomString(5)
    })));
}
async function createQuantity() {
    return await Promise.all(config.get('products').map((product, index) => {
...
async function createProducts() {
    ...
    return await Promise.all(products.map(({ reviews = [], useForChristmasSpecialChallenge = false, urlForProductTamperingChallenge = false, fileForRetrieveBlueprintChallenge = false, ...product }) => models.Product.create(product).catch((err) => {
...
async function createBaskets() {
    ...
    return await Promise.all(baskets.map(basket => {
...
async function createBasketItems() {
    ...
    return await Promise.all(basketItems.map(basketItem => {
...
async function createAnonymousFeedback() {
    ...
    return await Promise.all(feedbacks.map((feedback) => createFeedback(null, feedback.comment, feedback.rating)));
}
