...
exports.findChallengeByName = (challengeName) => {
...
exports.findChallengeById = (challengeId) => {
    for (const c in challenges) {
        if (Object.prototype.hasOwnProperty.call(challenges, c)) {
...
exports.solveFindIt = async function (key, isRestore) {
    const solvedChallenge = challenges[key];
    await models.Challenge.update({ codingChallengeStatus: 1 }, { where: { key, codingChallengeStatus: { [sequelize_1.Op.lt]: 2 } } });
...
exports.solveFixIt = async function (key, isRestore) {
    const solvedChallenge = challenges[key];
